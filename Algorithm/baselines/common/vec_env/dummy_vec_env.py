...
class DummyVecEnv(VecEnv):
    def __init__(self, env_fns):
        ...
        env = self.envs[0]
        VecEnv.__init__(self, len(env_fns), env.observation_space.spaces.get('observation'), env.action_space)
    ...
    def step_wait(self):
        for i in range(self.num_envs):
            ...
            if self.buf_dones[i]:
                obs_tuple = self.envs[i].reset().get('observation')
            if isinstance(obs_tuple, (tuple, list)):
                for t,x in enumerate(obs_tuple):
                    self.buf_obs[t][i] = x
            else:
                if isinstance(obs_tuple,np.ndarray):
                    self.buf_obs[0][i] = obs_tuple
                else:
                    self.buf_obs[0][i] = obs_tuple.get('observation')
